...
PDF = Path(__file__).resolve().parent #TODO upload file to site and extract infomation

#TOOLS FOR OUR LLMS
def extract_sections(text: str) -> dict:
    lines = text.split('\n')
    sections = {}
    current_section = "Header" # Default key for text before the first title
    current_content = []
    # Regex to match typical research headers (e.g., "Abstract", "1. Introduction", "METHODOLOGY")
    # Matches lines starting with optional numbers/roman numerals, followed by text, up to 100 chars
    header_pattern = re.compile(r"^(?:[0-9IVX]+\.?\s*)?[A-Z][a-zA-Z\s\-:,']{2,100}$") 
    for line in lines:
        clean_line = line.strip()
        if not clean_line:
            continue
        if header_pattern.match(clean_line):
            # Save the accumulated text to the PREVIOUS section's name
            if current_content:
                sections[current_section] = "\n".join(current_content)
            # Update the section name and reset the text accumulator
