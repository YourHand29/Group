...
import { analyzePaper, demoMap, demoPapers } from '../lib/research-orchestrator';
...
export default function Home() {
  ...
  const handleAnalyze = async () => {
    ...
    const result = await analyzePaper({ source: paperInput, fileName });
    setMap(result.map);
    setPapers((current) => [result.paper, ...current.filter((paper) => paper.code !== result.paper.code)]);
    setActivePaper(result.paper.id);
    setSelectedNode('thesis');
    setStatus('Map ready · 5 concepts connected');
      ...
      <section className="workspace">
        ...
        <div className="workspace-content">
          ...
          <section className="ingest-card" aria-label="Add a research paper">
            ...
            <div className="input-wrap"><span className="input-leading">↳</span><textarea id="paper-input" value={paperInput} onChange={(event) => { setPaperInput(event.target.value); setFileName(''); }} placeholder="Paste a paper link, abstract, or text…" rows={2} /><button type="button" className={`analyze-button ${isAnalyzing ? 'analyze-button-loading' : ''}`} onClick={handleAnalyze} disabled={isAnalyzing}>{isAnalyzing ? 'Mapping…' : 'Analyze paper'} <span>→</span></button></div>
          ...
          <div className="signal-grid">
            <section className="map-card" aria-label="Research concept map">
              <div className="map-card-head"><div><span className="card-eyebrow">RESPONSE <span>02</span></span><h2>Concept map</h2></div><div className="map-head-actions"><span className="confidence-pill"><i /> 92% confidence</span><button type="button" className="small-icon-button" aria-label="More map options">···</button></div></div>
            ...
            <aside className="insight-column">
              ...
              <section className="insight-card connection-card"><div className="connection-orbit"><span className="orbit-core">2</span><span className="orbit-dot orbit-dot-one" /><span className="orbit-dot orbit-dot-two" /></div><div><span className="card-eyebrow">CONNECTION</span><h3>Shared attention patterns</h3><p>Paper 01 and Paper 02 converge on efficient context mixing.</p><button type="button" className="text-button text-button-small" onClick={() => setActivePaper('paper-02')}>Explore overlap <span>→</span></button></div></section>
            </aside>
          </div>
